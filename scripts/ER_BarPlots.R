...
df2 <- read.csv("data/ER_MS1_20200403.csv", header = T, stringsAsFactors = F)

gdf2 <-  gather(df2, "group", "Expression", -AGI)

gdf2$tgroup <-  apply(str_split_fixed(gdf2$group, "_", 3)[, c(1, 2)], 1, paste, collapse ="_")

AVG_Count_ER <- gdf2 %>% group_by(AGI, tgroup) %>% summarize(expression_mean = mean(Expression)) %>% spread(., tgroup, expression_mean)

write.csv(AVG_Count_ER, "AVG_Count_ER.csv")



library(tidyverse)

df <- read_csv("data/ER_MS1_20200403.csv")
...
longdata2$Genotype <- factor(longdata2$Genotype,levels = c("WT", "atg2", "atg5", "atg7", "atg9"))
...
longdata2 %>% 
  ...
  scale_fill_manual(values=c("#008000","#B8860B","#4169E1",	"#FFA500","#DC143C"))+
